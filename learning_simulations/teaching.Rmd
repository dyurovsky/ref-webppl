...
propose_but_verify <- function(p = .6, C = 4, M = 16, n = 100) {
  ...
  for(i in 1:n) {
    ...
    if(!guess %in% trial_stims) {
      selected <- sample(trial_stims$object, 1)
      if(rbinom(1, 1, p))
        guess <- selected
      else
        guess <- 0
...
pbv_samples <- function(reps = 5000, p = .6, C = 4, M = 16, n = 100) {
  replicate(reps, propose_but_verify(p, C, M, n)) %>%
    enframe() %>%
    select(-name) %>%
    mutate(p = p) %>%
    rename(n = value)
...
pbv_props <- map(c(.25, .5, .75), ~pbv_samples(p = .x)) %>%
  bind_rows() %>%
  group_by(p, n) %>%
  summarise(prob = n()) %>%
  complete(p, n, fill = list(prob = 0)) %>%
  arrange(p, n) %>%
  mutate(prob = prob/sum(prob)) %>%
  mutate(prob = cumsum(prob)) 

# ggplot(pbv_props, aes(x = n, y = prob, color = as.factor(p))) + 
#   geom_line()
```
