...
outcomes_implicit_luce2<-webppl(program_file = "speaker.wppl", data=empiricalImplicitVocabs, data_var = "empiricalVocabs")

proc.time() - t1
# # 
# # ### clean up data from model
gamePredictions_luce2 <- outcomes_implicit_luce2 %>%
...
all_data_luce2 <- full_implicit %>%
  ...
  left_join(gamePredictions_luce2, by=c('ldf_num'='me', 'realLabel'='gameTrials', 'appearance')) %>%
...
```{r}
prop_methods_luce2 <- all_data_luce2 %>%
  filter(toBeDropped != 1) %>%
  rename(condition=speed) %>%
  # filter(partnersExposure == 0) %>%
  ungroup() %>%
