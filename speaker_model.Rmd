...
tmp <- exp3_dropped %>% group_by(condition, method) %>%
...
empiricalImplicitVocabs <- exp3_dropped %>%
...
all_data_model <- all_data %>%
  filter(ldf_num %in% gamePredictions_1.30_oldModel$me) %>%
  ...
  left_join(gamePredictions_1.30_oldModel, by=c('ldf_num'='me', 'realLabel'='gameTrials', 'appearance')) %>%
...
```{r}
prop_methods <- all_data_model_new %>%
  filter(toBeDropped != 1) %>%
  # rename(condition=speed) %>%
  # filter(partnersExposure == 0) %>%
  ungroup() %>%
